<div class="event-dates">
	<% if $Dates %>
		<h4><% if $Dates.Count > 1 %><span> Next Date:</span><% end_if %><% loop $Dates.limit(1) %>  $Format(l), $Format(F j) at $Format(g):$Format(i) $Format(A)<% end_loop %> </h4>
		...
		<div id="more-dates" data-dropdown-content class="f-dropdown content">
			<ul>
			<% loop $Dates.limit(11) %>
			<li>$Format(l), $Format(F j) $Format(g):$Format(i) $Format(A)</li>
			<% end_loop %>
			</ul>
		</div>
	<% else %>
